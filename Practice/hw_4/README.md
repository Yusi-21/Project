...
1. **Загрузка константы (LOAD_CONST):**
   - Команда занимает 5 байт.
   - Первые 8 бит: код операции `A=227 (0xE3)`.
   - Оставшиеся 32 бита: константа `B`.

2. **Чтение из памяти (READ_MEM):**
   - Команда занимает 4 байта.
   - Первые 8 бит: код операции `A=34 (0x22)`.
   - Оставшиеся 24 бита: адрес памяти `B`.

3. **Запись в память (WRITE_MEM):**
   - Команда занимает 4 байта.
   - Первые 8 бит: код операции `A=183 (0xB7)`.
   - Оставшиеся 24 бита: адрес памяти `B`.

4. **Унарная операция popcnt():**
   - Команда занимает 6 байт.
   - Первые 8 бит: код операции `A=1 (0x01)`.
   - Следующие 16 бит: смещение `B`.
   - Последние 24 бита: адрес `C`.
